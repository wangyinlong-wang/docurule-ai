...
> 实施状态（2026-08-12）：本文描述从 `v0.1` 工作版本演进到完整 MVP 的目标契约。当前仓库已经实现双文档医疗理赔 Demo、字段证据、姓名/金额确定性校验、人工决定、JSON 导出、Docker 与 Ollama；三文档费用报销模板、规则版本、乐观锁和独立审计实体属于下一阶段，不能视为现成功能。当前能力以 README 和自动化测试为准。
...
- 内置示例可稳定展示至少 3 种文档类型、8 个字段和 4 条规则，其中至少包含一条跨文档规则。
...
| 文档类型 | 字段 |
| --- | --- |
| 报销申请单 | `claimant_name`、`claim_date`、`claimed_amount`、`purpose` |
| 发票 | `invoice_no`、`invoice_date`、`buyer_name`、`seller_name`、`total_amount` |
| 支付凭证 | `payer_name`、`payee_name`、`paid_at`、`paid_amount` |
...
以下标准共同构成 0.1.0 发布门槛，不以“页面能打开”替代端到端验收。
...
- [ ] 仓库提供合成的费用报销材料、模板和 mock provider 响应；CI 无需真实模型即可走完整流水线。
- [ ] 上传 3 个样例文档后能识别为 `expense_claim`、`invoice`、`receipt`，展示至少 8 个结构化字段。
- [ ] 初次规则执行至少产生一个 `PASS` 和一个供演示的 `FAIL`/`WARN`，且页面能显示实际比较值。
- [ ] 修订导致失败的金额后自动重算，相关规则变为 `PASS`，旧结果仍可通过运行编号追溯。
- [ ] 批准案件后导出的 JSON 包含 3 个文档、当前字段、所有最新规则结果及字段修改审计事件。
